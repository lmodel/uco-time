...
ifneq ($(strip ${GEN_OARGS}),)
	$(RUN) gen-owl ${GEN_OARGS} -o $(DEST)/owl/uco_time.owl.ttl $(SOURCE_SCHEMA_PATH)
endif
...
ifneq ($(strip ${GEN_TARGS}),)
	...
	$(RUN) gen-typescript ${GEN_TARGS} $(SOURCE_SCHEMA_PATH) >${DEST}/typescript/uco_time.ts
endif
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/uco_time/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py project custom.env
